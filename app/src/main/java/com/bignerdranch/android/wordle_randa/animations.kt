...
/// shakes horizontally for each row,
fun shakeAnimation(layout: LinearLayout): () -> Unit {
    var lock = false  // what is lock
    return fun() {
        if (!lock) {
            lock = true
...
/// for when there is a valid answer, used for flipping the entire row, sequentially called
fun flipTextView(
    textView: TextView,
    colorTextView: Int,
    dur: Long = 80L,
...
//sequentially calling the animation
...
// calls the  slightlyScaleUpAnimation
...
// tells you the states and results
//the toast
